...
Spec: `tools/LOCAL-SITE.md`. There is also a hosted copy — `tools/build-pages.py --deploy`
publishes to GitHub Pages, and `LOCAL-SITE.md` §10 is its spec. A clone that wants its own
hosted copy must change `REPO` at the top of `build-pages.py` (§6).
...
python tools/save-watch.py --site http://127.0.0.1:8080
```
...
`--site` names — a local `serve-site.py`, or a hosted copy. Start the site first; the
watcher probes it and prints `reachable` or tells you it is not.

Two ports, neither started automatically: **8080** is the site, **8787** is the watcher.
If a page will not load, check both are up before looking anywhere else.
...
| a page 404s in the browser | check **both** 8080 and 8787 are running before anything else |
